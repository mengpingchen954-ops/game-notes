...
function App(){
 ...
 function create(){navigate(()=>{updatePlanUrl('editor');setEditing({record:blankRecord(),isNew:true});setView('editor');setMobile(false);setDirty(false);window.scrollTo(0,0);});}
 ...
 const viewTitle=view==='detail'?record?.title:view==='editor'?'编辑分析':view==='plan'?'90天执行计划':studioTitle||(view==='template'?'分析模板':view==='favorites'?'我的收藏':view==='drafts'?'分析草稿':'全部分析');
 ...
 {view==='editor'&&editing?<Editor key={editing.record.id} {...editing} onSave={save} onCancel={()=>navigate(()=>{setEditing(null);setView(record?'detail':'library');})} onDirty={()=>setDirty(true)}/>:view==='plan'?<PlanView onView={changeView}/>:note?<KnowledgeView key={note.key} note={note} onView={changeView}/>:view==='template'?<TemplateView onNew={create}/>:studioModules.some(item=>item.key===view)?<StudioGuide mode={view} onToast={setToast} onView={changeView}/>:view==='detail'&&record?<GameDetail record={record} onEdit={edit} onFavorite={()=>{if(persist(records.map(r=>r.id===record.id?{...r,favorite:!r.favorite}:r)))setToast(record.favorite?'已取消收藏':'已加入收藏');}} onMarkdown={()=>download(markdown(record),record.title.replace(/[\\/:*?"<>|]/g,'-')+'.md','text/markdown')} onDelete={remove} onDuplicate={duplicate}/>:<LibraryView key={view} records={records} mode={view} onSelect={select} onNew={create}/>}
